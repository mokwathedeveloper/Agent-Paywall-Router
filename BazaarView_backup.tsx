...
import { Search, FileText, BarChart3, Zap, X, Trophy, Plus, Globe, Shield, Coins } from "lucide-react";
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div>
          ...
          <p className="body" style={{ color: "var(--text-muted)", fontSize: "0.875rem" }}>
            Agent-native marketplace for Stellar x402 services.
          </p>
        </div>
        ...
        {services.length > 0 && (
          <div style={{
            ...
            <div>
              ...
              {services[0] && `Selected ${services[0].name} ($${services[0].priceUsd.toFixed(2)}) as optimal entry point.`}
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ 
        ...
        {/* Tools Section */}
        <div>
          <div className="caption" style={{ marginBottom: "var(--s5)", display: "flex", alignItems: "center", gap: "var(--s2)" }}>
            <Globe size={14} /> Available Services
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <Shield size={24} color="var(--emerald)" />
          </div>
          <div>
            ...
            <p className="body" style={{ fontSize: "0.875rem", margin: 0 }}>
              ...
              Our agent logic always prioritizes the most cost-effective verifiable service.
            </p>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ maxWidth: "800px" }}>
            <div className="badge badge-indigo" style={{ marginBottom: "var(--s4)" }}>PROVIDER PROGRAM</div>
            <h2 className="h2" style={{ marginBottom: "var(--s3)", fontSize: "1.875rem" }}>Become a Service Provider</h2>
            <p className="body-lg" style={{ marginBottom: "var(--s8)" }}>
              List your own AI service and earn USDC on every request.
              The Agent Paywall Router handles the payment flow and provides transaction proofs for you.
            </p>
            ...
            <form
              ...
              style={{
                display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--s6)",
                background: "var(--bg-surface)", padding: "var(--s8)",
                borderRadius: "var(--r-xl)", border: "1px solid var(--border-strong)",
                boxShadow: "0 20px 40px rgba(0,0,0,0.4)"
              }}
            >
              <div style={{ gridColumn: "1 / -1" }}>
                <div className="caption" style={{ marginBottom: "var(--s2)" }}>Service Name</div>
                <input name="name" className="input" placeholder="e.g. GPT-4o Summarizer" required />
              </div>
              <div>
                <div className="caption" style={{ marginBottom: "var(--s2)" }}>Price (USDC)</div>
                <div style={{ position: "relative" }}>
                  <div style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)" }}>$</div>
              ...
              <div>
                ...
                <select name="protocol" className="input" style={{ appearance: "none", cursor: "pointer" }}>
                  <option value="x402">x402 (HTTP Standard)</option>
                  <option value="mpp">MPP (Session-based)</option>
                </select>
              </div>
              <div style={{ gridColumn: "1 / -1" }}>
                <div className="caption" style={{ marginBottom: "var(--s2)" }}>API Endpoint (URL)</div>
                <input name="endpoint" className="input" placeholder="https://your-api.com/v1/summarize" required />
              </div>
              <div style={{ gridColumn: "1 / -1" }}>
                <div className="caption" style={{ marginBottom: "var(--s2)" }}>Description</div>
                <textarea name="description" className="input" placeholder="Explain what your tool does..." style={{ minHeight: "100px", resize: "none" }} required />
              </div>
              <button type="submit" className="btn btn-primary btn-lg" style={{ gridColumn: "1 / -1", marginTop: "var(--s2)" }}>
                <Plus size={18} /> Register Service in Marketplace
              </button>
            </form>
          </div>
          ...
          <div style={{
            position: "absolute", top: "50%", right: "-10%", width: "40%", height: "80%",
          ...
          >
            <motion.div
              ...
              style={{
                ...
                borderRadius: "var(--r-xl)", width: "100%", maxWidth: 640,
                maxHeight: "85vh", overflow: "hidden",
                boxShadow: "0 32px 64px rgba(0,0,0,0.5)",
              }}
            >
              <div style={{
                padding: "var(--s6) var(--s8)", borderBottom: "1px solid var(--border-dim)",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "var(--s4)" }}>
                  <div style={{
                    ...
                    <Coins size={20} color="var(--emerald)" />
                  </div>
                  <div>
              ...
              <div style={{ padding: "var(--s8)", overflowY: "auto", maxHeight: "65vh" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "var(--s4)", marginBottom: "var(--s8)" }}>
                  {[
                    ...
                    { label: "Asset", value: "USDC (Testnet)", mono: false },
                  ].map((f) => (
                    <div key={f.label} style={{
                      ...
                      {f.cost
                        ? <div className="cost" style={{ fontSize: "1rem" }}>{f.value}</div>
                        : <div style={{ fontWeight: 700, fontSize: "1rem", fontFamily: f.mono ? "var(--font-mono)" : undefined }}>{f.value}</div>}
                    </div>
                  ))}
                </div>
                ...
                <div>
                  ...
                  <pre style={{
                    padding: "var(--s5)", background: "var(--bg-deep)",
