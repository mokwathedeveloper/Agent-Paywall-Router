...
export default function A2APage() {
  ...
  const addLog = (action: string, status: "success" | "failed", detail: string) => {
    setLogs((prev) => [...prev, { action, status, detail }]);
  };

  const runA2ADemo = async () => {
    if (!secretKey.startsWith("S") || secretKey.length !== 56) {
      alert("Please enter a valid Stellar Secret Key (starts with S, 56 characters).");
      return;
    }
    ...
    try {
      ...
      const body = await res.json().catch(() => ({} as A2AResult)) as A2AResult;

      if (!res.ok || body.ok === false) {
        const err = body?.error ?? { code: "execution_failed", message: "A2A run failed." };

        if (err.code === "trustline_missing") {
          addLog("Trustline Missing", "failed", err.message);
          setResult({ action: "fix_trustline", message: err.message });
          return;
        }
        if (err.code === "invalid_key") {
          addLog("Checksum Error", "failed", err.message);
          setResult({ action: "invalid_key", message: err.message });
          return;
        }

        addLog("Execution Info", "success", "Ensure your account is funded on Testnet with USDC.");
        addLog("Status", "failed", err.message);
        setResult({ action: "execution_failed", message: err.message });
        return;
      }
  ...
  const fixTrustline = async () => {
    ...
    try {
      ...
      const body = await res.json().catch(() => ({} as TrustlineResult)) as TrustlineResult;
      ...
      addLog("Setup Failed", "failed", msg);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  return (
    <div className="container" style={{ maxWidth: 800, padding: "var(--s10) var(--s4)" }}>
      <div style={{ marginBottom: "var(--s8)" }}>
        ...
        <p className="body" style={{ color: "var(--text-dim)", marginTop: "var(--s3)" }}>
          This page demonstrates a &ldquo;Second Agent&rdquo; using our MCP server to autonomously discover and pay for tools.
      ...
      <div className="card" style={{ padding: "var(--s6)", background: "var(--bg-deep)" }}>
        <div style={{ marginBottom: "var(--s6)" }}>
          ...
          <div style={{ display: "flex", gap: "var(--s3)" }}>
            ...
            />
            <button className="btn btn-primary" onClick={runA2ADemo} disabled={isRunning || !secretKey}>
        ...
        <div style={{
          background: "#000", borderRadius: "var(--r-lg)", padding: "var(--s4)",
          fontFamily: "var(--font-mono)", fontSize: "0.8125rem", border: "1px solid var(--border-dim)",
        }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "var(--s2)" }}>
            {logs.length === 0 && <div style={{ color: "#444" }}>Waiting for agent initiation…</div>}
            {logs.map((log, i) => (
              <div key={i} style={{ display: "flex", gap: "var(--s4)" }}>
                <span style={{ color: log.status === "failed" ? "var(--rose)" : "var(--emerald)", width: 100 }}>
        ...
        {result && (
          <div style={{
            ...
            ) : result.action === "invalid_key" || result.action === "execution_failed" ? (
              <div style={{ fontWeight: 600, color: "var(--rose)", fontSize: "0.875rem" }}>
                ✗ {result.message}
              </div>
            ) : (
              <>
